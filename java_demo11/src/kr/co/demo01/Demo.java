...
public class Demo {
	...
	public static void main(String[] args) {
		 ...
		 *	  - 배열에 데이터를 담아 관리하는 방법에서 좀 더 편하게 
		 *		데이터를 추가/수정/삭제 할 수 있는 방법을 제공
		 ...
		 *		  - 한 가지 타입에 대해서만 저장할 수 있다.
		 *
		 *		컬렉션 특징
		 *		  - 언제든 크기를 늘리거나 줄일 수 있다.
		 *		  - 컬렉션에 저장된 데이터의 추가/수정/삭제 등의 작업이
		 *			컬렉션에서 제공하는 메서드에 의해 처리된다.
		 *		  - 여러 타입에 대한 데이터의 저장이 가능하다.
		 */
		
		/*
		 * 	List 컬렉션
		 * 	  - 데이터를 순차적으로 나열한 자료구조로 인덱스로 관리한다.
		 * 	  - 중복 데이터 저장이 가능하다.
		 * 	  - 구현 클래스로 ArrayList, Vector, LinkedList가 있다.
		 * 
		 * 	ArrayList
		 * 	  - List의 구현체로 저장 용량이 10에서 데이터가 추가/삭제될 때마다
		 * 		자동으로 늘어나면서 데이터를 관리한다.
		 * 	  - Thread Safe 기능을 제공하지 않는다.
		 * 
		 * 	Vector
		 * 	  - List의 구현체로 ArrayList와 동일하며, Thread Safe 기능을 제공한다.
		 * 
		 * 	LinkedList
		 * 	  - List의 구현체로 특정 위치에서 객체를 추가하거나 제거하면
		 * 		앞/뒤 노드의 링크 정보만 변경하는 형태로 동작하기 때문에 
		 * 		빈번한 데이터 추가/삭제 작업이 이루어지는 경우 성능이 가장 우수한 컬렉션
		 */
		
		ArrayList<Integer> aList = new ArrayList<Integer>();
//		<> <- 컬렉션으로 관리할 데이터 타입을 작성(제네릭타입)
		
//		데이터 추가 : 가장 마지막 순서에 추가
		aList.add(10); aList.add(20); aList.add(30); aList.add(40); aList.add(50);
