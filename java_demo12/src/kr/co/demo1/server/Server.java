...
public class Server {
	...
	public static void main(String[] args) {
		 ...
		 * 	네트워크 프로그램
		 * 	  - 네트워크로 연결된 컴퓨터 장치 간에 
		 * 		데이터를 송/수신할 수 있도록 만드는 프로그램
		 * 	  - 서버/클라이언트 나누어서 프로그램을 작성하게 된다.
		 * 	  - 네트워크 연결이 필요한 정보로 IP 주소와 Port 번호가 있으며,
		 * 		이 정보를 통해 연결할 서버와 클라이언트를 식별하게 된다.
		 * 	  - 네트워크 연결에는 UDP/TCP 2 가지 방식의 연결 방법이 있으며
		 * 		UDP의 경우 데이터 통신의 신뢰성이 낮은 반면, 전송속도가 빠르다는 특징이 있고
		 * 		TCP의 경우 데이터 통신의 신뢰성이 높으며 대신 전송속도가 느리다는 특징이 있다.
		 * 
		...
		try {
			...
			System.out.println("클라이언트가 접속할 수 있도록 연결 소켓을 생성합니다.");
			while (true) {
				byte[] data = new byte[1024];
				DatagramPacket dPacket = new DatagramPacket(data, data.length);
				System.out.println("클라이언트의 연결을 대기합니다.");
