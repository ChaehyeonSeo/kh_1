/*
 *	DDL
 *	  - 데이터 정의어로 데이터를 저장하고 다루기 위한
 *		객체를 생성하고(CREATE) 수정하고(ALTER) 삭제(DROP)하는 구문을 말한다.
 *	  - ORACLE에서 사용하는 객체들로는 TABLE, VIEW, SEQUENCE, USER, PROCEDUAL, FUNCTION 등이 있다.
 */
 ...
 *
 *	데이터 타입
 *		CHAR(크기) : 고정 크기 (바이트) 문자 데이터, 최대 2,000 바이트 저장 가능
 *		CHAR(CHAR 크기) : 문자를 기준으로 크기 설정
 *		VARCHAR2(크기) : 가변 크기 문자 데이터, 최대 4,000바이트 저장 가능
 *						 한글은 3바이트
 *		VARCHAR2(CHAR 크기) : 문자를 기준으로 크기 설정
 *		NUMBER : 숫자 데이터(최대 40자리)
 *		NUMBER(길이) : 숫자 데이터, 길이 지정 가능
 *		NUMBER(길이, 자릿수) : 숫자 데이터, 소수점 자릿수까지 지정
 *		DATE : 날짜 데이터
 *		TIMESTAMP : 타임스탬프 데이터
 *		LOB : 가변길이 바이너리 데이터, 최대 2GB까지 저장 가능
 *		LONG : 가변길이 문자 데이터, 최대 2GB까지 저장 가능
 *		BLOB : 대용량 데이터 저장용 객체, 4GB까지 저장 가능 (바이너리 데이터)
 *		CLOB : 대용량 데이터 저장용 객체, 4GB까지 저장 가능 (문자 데이터)
 *
 *	제약 조건
 *		NOT NULL : NULL 데이터 저장을 허용하지 않음
 *		UNIQUE : 중복값 저장을 허용하지 않음
 *		PRIMARY KEY : NULL, 중복값 저장을 모두 허용하지 않음
 *					  (Record 고유 식별값으로 사용하기 위해)
 *		FOREIGN KEY : 참조하는 테이블의 컬럼 값이 존재하면 허용
 *					  (외래키 : 다른 테이블의 값을 참조하여 관계 형성)
 *		CHECK : 지정한 값만 저장할 수 있음
 *		DEFAULT : 기본값을 설정하여 데이터 추가 작업에 누락이 되어 있어도 기본값이 저장되게 한다.
 *				  제약 조건은 아니지만 제약 조건이 작성되는 부분에 사용
 *	
 *	위의 제약조건을 위반하는 데이터 추가/수정/삭제 작업이 이루어지는 경우,
 *	"제약조건 위반"이라는 에러가 발생하게 된다. 	
...
CREATE TABLE TEST2_T (
	 ...
	 , CONSTRAINT TEST2_T_COL_NAME3_PK FOREIGN KEY(COL_NAME3) REFERENCES REF_T(REF_COL1)
	 , CONSTRAINT TEST2_T_COL_NAME4_PK UNIQUE(COL_NAME5)
);
...
INSERT INTO TEST1_T VALUES(2, 'C', 'A', TO_DATE(20221114));
...
/*
 *	ALTER TABLE : 생성한 테이블의 이름을 변경하거나 컬럼명, 데이터 타입, 제약 조건을 수정할 때
 *				  사용 테이블에 데이터가 저장되어 있는 경우 수정 작업이 안될 수 있다.(데이터 타입)
 */
CREATE TABLE TEST3(
	   COL1 NUMBER
	 , COL2 CHAR(10)
...
ALTER TABLE TEST4 MODIFY COL5 CONSTRAINT TEST_COL5_NN NOT NULL;
