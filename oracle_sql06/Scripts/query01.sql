...
/* DBeaver에서는 적용 안됨. sqlplus에서 출력 기능을 사용하기 위해 필요함 */
-- SET SERVEROUTPUT ON;
...
BEGIN
	SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
	  INTO EMP_ID, FNAME, LNAME	/* 조회된 값을 변수에 저장하기 위한 INTO절 */
	  FROM WEB_ADMIN.EMPLOYEES
	 WHERE EMPLOYEE_ID = :ID;	/* SQLplus에서는 &ID 형식 사용 */
...
BEGIN
	NUM := :NUM;
	IF(NUM > 10) THEN
		DBMS_OUTPUT.PUT_LINE('변수 NUM에 저장된 값은 10 보다 큽니다.');
	ELSIF(NUM = 10) THEN
		DBMS_OUTPUT.PUT_LINE('변수 NUM에 저장된 값은 10입니다.');
	ELSE
		DBMS_OUTPUT.PUT_LINE('변수 NUM에 저장된 값은 10 보다 작습니다.');
	END IF;
END;
...
BEGIN
	VID := :ID;
	NAME := :NAME;	/* 작은 따옴표를 입력 창에 직접 입력할 것 PLSQL에서는 '&NAME'이 되기 때문에 필요없음 */
	AGE := :AGE;
	SELECT COUNT(ID)
	  INTO CNT
	  FROM TEST1
...
SELECT * FROM TEST1;
INSERT INTO TEST1 VALUES(2, 'NAME', 2);