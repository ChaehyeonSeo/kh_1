 ...
 *
 *	USERNAME과 PASSWORD를 입력받을 수 있는 PROCEDURE를 작성하여
 *	올바른 정보를 입력한 경우 '로그인 성공'을 출력하고
 *	별도의 LOGIN_LOG 테이블에 USERNAME, 로그인 성공 날짜 정보가 기록될 수 있게 한다.
 *	
 *	만약 올바른 정보를 입력하지 않은 경우 '로그인 실패'를 출력하고 
 *	로그인 시도 횟수가 저장되는 컬럼에 값을 +1 증가시킨다.
 *	
 *	로그인 시도 횟수는 최대 3번이며, 3번째에서 실패한 경우 계정은 잠금상태가 되어
 *	5분간 로그인 시도를 할 수 없게 한다.
 *	(5분이 지난 후에는 다시 로그인을 시도할 수 있다.)
 *
 *	로그인 시도 횟수는 로그인이 성공했을 때 다시 0으로 초기화되어야 한다.
 */
CREATE TABLE LOGIN(
	...
	ELSE 
		...
		ELSE 
			...
			ELSE 
				...
				INSERT INTO LOGIN_LOG VALUES(SEQ_LOGIN_LOG.NEXTVAL, IN_USERNAME, 'FAIL', SYSDATE);
