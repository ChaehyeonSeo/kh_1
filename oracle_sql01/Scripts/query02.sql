...
/*
 * 	- SELECT 구문에는 WHERE 절 ORDER BY 절 등이 추가로 사용된다.
 * 	- FROM 절에는 조회할 테이블명을 작성하게 된다.
 * 	- WHERE 절에는 FROM에 명시한 테이블에서 데이터를 조회하기 위한 조건을 작성하게 된다.
 * 	- ORDER BY 절에는 조회 결과에 대한 정렬 기준을 작성하게 된다.
 * 	- SELECT 절에는 FROM 절에 명시한 테이블의 컬럼을 작성하게 된다.
 * 	  이 때 작성된 컬럼의 데이터들만 Record Set으로 반환이 된다.
 * 	- *은 모든 컬럼을 의미
 */
...
/*
 *	SELECT 절에 컬럼명과 리터럴 값을 혼합하여 
 *	Record Set으로 반환되는 결과값을 변경할 수 있다.
 */
...
/*
 *	SELECT 절에 컬럼과 컬럼을 결합하여 하나의 컬럼으로 만들어
 *	Record Set을 반환할 수 있다.
 */
 ...
 *		AND : 모든 결과가 true일 때 true 반환
 *		OR : 하나의 결과가 true일 때 true 반환
 *		NOT : 반환된 결과에 대한 부정
 */
SELECT FIRST_NAME
	 , LAST_NAME
