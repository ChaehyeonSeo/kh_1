...
public class Demo04 {
	...
	public static void main(String[] args) {
		 ...
		 * 	}
		 * 
		 *	초기식 : 변수를 초기화하기 위한 식 (생략 가능)
		 *		    반복문이 시작할 때 최초 1회만 동작함
		 *	조건식 : 반복문을 계속 수행할지에 대한 조건 (생략 가능)
		 *			조건식의 결과가 참이면 반복문 안의 코드 실행
		 *			조건식의 결과가 거짓이면 반복문 중단하고 다음으로 넘어감
		 *			반복문이 시작할 때 뿐만 아니라 반복이 이루어지면 계속 실행
		 *	증감식 : 보통 초기식에서 초기화한 변수의 값을 증감시키기 위해 사용 (생략 가능)
		 *			반복문의 마지막 코드가 실행된 후에 수행하는 코드
		 *			반복문이 시작할 때에는 동작 않고 반복이 이루어지면 계속 실행
		 *
		 *	for 반복문에서 초기식, 조건식, 증감식을 모두 생략하면 무한 반복
		 *	for 반복문에서 조건식을 잘못 설정하면 무한 반복
		 *
...
//		int x;
//		for(x = 1; x <= 3; x++) {
//			System.out.printf("반복 횟수 및 변수 x에 저장된 값 -> %d회 / %d%n", x, x);
//		}
//		System.out.printf("변수 x에 저장된 최종 값 -> %d%n", x);
	
//		for(int x2 = 1; x2 <= 3; x2++) {
//			System.out.printf("반복 횟수 및 변수 x2에 저장된 값 -> %d회 / %d%n", x, x);
//		}
//		System.out.printf("변수 x2에 저장된 최종 값 -> %d%n", x2);
		
//		for(int x3 = 1; x3 <= 3;) {
//			System.out.printf("%d 번째 반복%n", x3);
//			x3++;
//		}
